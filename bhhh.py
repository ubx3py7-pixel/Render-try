#!/usr/bin/env python3
# bhhhh.py  — Part 1 of 2 (Manager + Persistence + Manager handlers)

"""
Manager (minimal) + Worker (heavy bot) host — Reworked version
...
# ---------------------------
# CONFIG – edit locally or set env var MANAGER_TOKEN
# ---------------------------
# Hardcoded fallback (you can keep it) — recommended to use env var in production
_HARDCODED_MANAGER_TOKEN = "8516170208:AAGgAZ33jrmxkRaDxKmE6A64wVKphC30qp4"

MANAGER_TOKEN = os.environ.get("MANAGER_TOKEN", _HARDCODED_MANAGER_TOKEN)
OWNER_ID = int(os.environ.get("OWNER_ID", "6940098775"))

# Optional persistent data directory (Render disk mount)
DATA_DIR = os.environ.get("DATA_DIR", "")  # e.g. /opt/render/project/data
...
def load_json(name: str, default):
    try:
        ...
        logging.exception("Failed reading %s — using default", name)
    return default
...
def save_json(name: str, data):
    try:
        ...
        logging.exception("Failed saving %s", name)
...
async def m_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
@only_owner
async def m_adduser(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.reply_to_message:
        ...
        try:
            ...
            return await update.message.reply_text("Bad id.")
    else:
...
@only_owner
async def m_deluser(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.reply_to_message:
        target = update.message.reply_to_message.from_user.id
    elif context.args:
        try:
...
@only_owner
async def m_listusers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not USERS:
...
@invited_only
async def m_addapi(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def m_handle_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # start worker
    try:
        # build_worker_app defined in Part 2 below
...
# build manager (defined later) — handler registration helper
def build_manager_app(token: str) -> Application:
    ...
    return app
    # bhhhh.py — Part 2 of 2 (Worker builder + Startup & Shutdown)
# Continue in the same file after Part 1 content
...
# ---------------------------
async def start_all():
    print("=== START_ALL: entered ===")
    logging.info("=== START_ALL: entered ===")

    # quick manager token validation
    try:
        bot_check = TgBot(MANAGER_TOKEN)
        me = await bot_check.get_me()
        print(f"=== MANAGER_TOKEN OK: @{me.username} ===")
        logging.info("Manager token validated for @%s", me.username)
    except Exception as e:
        print("=== MANAGER_TOKEN FAIL:", e)
        logging.exception("Manager token validation failed")
        raise

    # ensure manager token is not placeholder (if you kept placeholder check)
    global manager_app
...
if __name__ == "__main__":
    ...
    try:
        ...
        _loop.run_until_complete(shutdown_all())
        save_all()
